...
W_out = tf.Variable(tf.truncated_normal([30,10], stddev=0.1))
...
Y = tf.nn.softmax(tf.matmul(L3, W_out) + b_out)
...
allweights = tf.concat([tf.reshape(W1, [-1]), tf.reshape(W2, [-1]), tf.reshape(W3, [-1]), tf.reshape(W_out, [-1])], 0)
allbiases  = tf.concat([tf.reshape(b1, [-1]), tf.reshape(b2, [-1]), tf.reshape(b3, [-1]), tf.reshape(b_out, [-1])], 0)
